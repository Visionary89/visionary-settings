...
prompt_git() {
  ...
  if [[ -d .git ]]; then
      local git_color_segment tmp_git_promt
      tmp_git_prom=$(__git_ps1 "%s")
      if echo x"$tmp_git_prom" | grep '*\|+' > /dev/null; then
        git_color_segment='yellow'
      else
        git_color_segment='green'
      fi
    prompt_segment $git_color_segment $CURRENT_FG $(__git_ps1 "%s");
  fi
}
